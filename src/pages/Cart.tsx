  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-28 pb-12 px-4 sm:px-6 lg:px-12 max-w-[1440px] mx-auto">
        ...
        <p className="font-body text-muted-foreground mb-10">{count} {count === 1 ? "item" : "items"}</p>
            ...
            <div className="lg:col-span-2 space-y-6">
              {items.map((item) => (
                <div key={`${item.id}-${item.size}`} className="flex gap-5 p-4 rounded-2xl border border-border bg-card">
                  <Link to={`/product/${item.id}`} className="w-24 h-28 sm:w-32 sm:h-36 rounded-xl overflow-hidden bg-secondary flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button onClick={() => removeItem(item.id, item.size)} className="p-2 text-muted-foreground hover:text-destructive transition-colors">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center border border-border rounded-full">
                        <button onClick={() => updateQuantity(item.id, item.size, item.quantity - 1)} className="p-2 hover:bg-secondary rounded-l-full transition-colors">
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="px-4 font-display text-sm font-medium">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.size, item.quantity + 1)} className="p-2 hover:bg-secondary rounded-r-full transition-colors">
